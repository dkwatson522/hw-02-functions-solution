...
function wordCounter (phrase) {
  ...
  const wordCount = {} // object that we'll use to keep track of word occurences

  phraseWords.forEach((word) => {
    /* Plan
      1. check of word is already in wordCount
       - if word is not in wordCount then add it as a key and set value to 1
       - if word is already in wordCount then then increment value of word (key) by 1
    */

    if (wordCount[word] === undefined) {
      // word does not exist in wordCount
      wordCount[word] = 1
    } else {
      // when word already exists in wordCount
      wordCount[word] += 1
    }
  })
...
//   const phraseWords = phrase.split(' ')
//
//   return phraseWords.reduce((wordCount, word) => {
//     if (wordCount[word] === undefined) {
